...
Velocity guard for the VLA -> Husky pipeline.

    VLA --> action_to_cmdvel_node --> /cmd_vel_raw --> [safety_layer_node] --> /cmd_vel --> twist_mux
...
VLA re-plan from the new frame (deviation via re-planning, not via steering).
